...
const StoryApi = {
    ...
    async stroyStream(session, itineraryId, step) {
        const params = new URLSearchParams({ session, itineraryId, step });

        const responseStream = await fetch(`${import.meta.env.VITE_API_URL}/storyStream?${params}`, {
            method: 'GET'
        });
