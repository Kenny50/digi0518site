...
import MysteryApi from "../api/MysteryApi";

export default {
    setup() {
        const itinerary = ref([]);
        const itineraryId = 178;
        ...
        const checkInLocation = async () => {
            try {
                const location = await MysteryApi.checkInLocation(
                    state.value.session,
                    itineraryId,
                    state.value.step,
        ...
        const startItinerary = async () => {
            const newItinerary = await MysteryApi.startItinerary(itineraryId);
        ...
        watch(() => state.value.step, async (newStep) => {
            itinerary.value.push({
                ...
                step: newStep,
                story: "",
                attractionName: state.value.attractionName + "--已簽到",
